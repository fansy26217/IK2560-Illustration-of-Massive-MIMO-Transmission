function pictures2 = phase2(maximum_time, angle, receive_i_1, receive_i_2, receive_num_1, receive_num_2, station_px, station_py, user_px, user_py, delta_t, v)
%% set parameters 参数设定
t_max = maximum_time;
t = 0:delta_t:t_max;
...
for i = 1:receive_num_2
    ...
    [vector_3(i,1),vector_3(i,2)] = pol2cart(rho_2(i),r);
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
vector_3 = zeros(receive_num_1,2);
...
p_3 = zeros(receive_num_2,2*length(t));
p_3(:,1:2) = repmat([user_px(2),user_py(2)],receive_num_2,1);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% for use1 对于用户1
    for i = 1:receive_num_1      
        ...
        if(inpolygon(p_next_1(1),p_next_1(2),x_block1,y_block1)==1)    % reflected by block1 触碰到障碍物1反弹
            [P1,P2] = find_edge(position_1,p_next_1,x_block1,y_block1);    % the edge that the signal arrives 判断在哪条边上反弹
            vector_1(i,:) = reflection(vector_1(i,:),P1,P2);    % execute reflection 执行反射函数
      
        elseif(inpolygon(p_next_1(1),p_next_1(2),x_block2,y_block2)==1)    % reflected by block2 触碰到障碍物2反弹
            [P1,P2] = find_edge(position_1,p_next_1,x_block2,y_block2);
            vector_1(i,:) = reflection(vector_1(i,:),P1,P2);
        
        elseif(inpolygon(p_next_1(1),p_next_1(2),x_block3,y_block3)==1)    % reflected by block2 触碰到障碍物2反弹
            [P1,P2] = find_edge(position_1,p_next_1,x_block3,y_block3);
            vector_1(i,:) = reflection(vector_1(i,:),P1,P2);

        elseif(inpolygon(p_next_1(1),p_next_1(2),x_block4,y_block4)==1)    % reflected by block2 触碰到障碍物2反弹
            [P1,P2] = find_edge(position_1,p_next_1,x_block4,y_block4);
            vector_1(i,:) = reflection(vector_1(i,:),P1,P2);

        elseif(inpolygon(p_next_1(1),p_next_1(2),x_block5,y_block5)==1)    % reflected by block2 触碰到障碍物2反弹
            [P1,P2] = find_edge(position_1,p_next_1,x_block5,y_block5);
            vector_1(i,:) = reflection(vector_1(i,:),P1,P2);

        elseif(inpolygon(p_next_1(1),p_next_1(2),x_block6,y_block6)==1)    % reflected by block2 触碰到障碍物2反弹
            [P1,P2] = find_edge(position_1,p_next_1,x_block6,y_block6);
            vector_1(i,:) = reflection(vector_1(i,:),P1,P2);

        elseif(inpolygon(p_next_1(1),p_next_1(2),x_block7,y_block7)==1)    % reflected by block2 触碰到障碍物2反弹
            [P1,P2] = find_edge(position_1,p_next_1,x_block7,y_block7);
            vector_1(i,:) = reflection(vector_1(i,:),P1,P2);

        elseif((p_next_1(1)-station_px)^2 + (p_next_1(2)-station_py)^2 <=0.1)    % received by the base station 被基站接受
    ...
    for i = 1:receive_num_3        
        if((p_1(i,2*n-1)==station_px && p_1(i,2*n)==station_py)||(p_1(i,2*n-1)==12 && p_1(i,2*n)==12))    % 接收到的信号不再绘制
            p_3(i,2*(n+1)-1) = 12;
            p_3(i,2*(n+1)) = 12;
            continue
        end
        ...
        if(inpolygon(p_next_3(1),p_next_3(2),x_block1,y_block1)==1)    % reflected by block1 触碰到障碍物1反弹            
            [P1,P2] = find_edge(position_3,p_next_3,x_block1,y_block1);    % the edge that the signal arrives 判断在哪条边上反弹
            vector_3(i,:) = reflection(vector_3(i,:),P1,P2);    % execute reflection 执行反射函数
      
        elseif(inpolygon(p_next_3(1),p_next_3(2),x_block2,y_block2)==1)    % reflected by block2 触碰到障碍物2反弹
            [P1,P2] = find_edge(position_3,p_next_3,x_block2,y_block2);
            vector_3(i,:) = reflection(vector_3(i,:),P1,P2);
        
        elseif(inpolygon(p_next_3(1),p_next_3(2),x_block3,y_block3)==1)    % reflected by block2 触碰到障碍物2反弹
            [P1,P2] = find_edge(position_3,p_next_3,x_block3,y_block3);
            vector_3(i,:) = reflection(vector_3(i,:),P1,P2);

        elseif(inpolygon(p_next_3(1),p_next_3(2),x_block4,y_block4)==1)    % reflected by block2 触碰到障碍物2反弹
            [P1,P2] = find_edge(position_3,p_next_3,x_block4,y_block4);
            vector_3(i,:) = reflection(vector_3(i,:),P1,P2);

        elseif(inpolygon(p_next_3(1),p_next_3(2),x_block5,y_block5)==1)    % reflected by block2 触碰到障碍物2反弹
            [P1,P2] = find_edge(position_3,p_next_3,x_block5,y_block5);
            vector_3(i,:) = reflection(vector_3(i,:),P1,P2);

        elseif(inpolygon(p_next_3(1),p_next_3(2),x_block6,y_block6)==1)    % reflected by block2 触碰到障碍物2反弹
            [P1,P2] = find_edge(position_3,p_next_3,x_block6,y_block6);
            vector_3(i,:) = reflection(vector_3(i,:),P1,P2);

        elseif(inpolygon(p_next_3(1),p_next_3(2),x_block7,y_block7)==1)    % reflected by block2 触碰到障碍物2反弹
            [P1,P2] = find_edge(position_3,p_next_3,x_block7,y_block7);
            vector_3(i,:) = reflection(vector_3(i,:),P1,P2);
            
        elseif((p_next_3(1)-station_px)^2 + (p_next_3(2)-station_py)^2 <=0.1)    % received by the base station 被基站接受
...
hold on
s2 = scatter(p_3(:,2*length(t)-1),p_3(:,2*length(t)),10,"filled","b");
s3 = scatter(p_3(:,2*length(t)-1),p_3(:,2*length(t)),10,"filled","b");
...
for n = length(t):-1:1
    ...
    for i = 1:receive_num_2
        revp_2(i,1:2) = [p_3(i,2*n-1),p_3(i,2*n)];
    end
    hold on
    s1.XData = revp_1(:,1);
    s1.YData = revp_1(:,2);
